...
class FFTTest(c:FFT) extends PeekPokeTester(c)
  ...
  for (t <- 0 until iterNum) {
    ...
    var ovNum1: Int = 0
    var eps: Double = 1e-9
    for (i <- 0 until FFTLength / 4) {
      var ref1 = ref(reverse(i * 4, log2Ceil(FFTLength)))
      ...
      var ref2 = ref(reverse(i * 4 + 1, log2Ceil(FFTLength)))
      ...
      var ref3 = ref(reverse(i * 4 + 2, log2Ceil(FFTLength)))
      var d3 = peek(c.io.dOut3)
      error1 = math.abs((((2 * d3("re").toDouble / bound) - ref3.re) / (ref3.re + eps) + ((2 * d3("im").toDouble / bound) - ref3.im) / (ref3.im + eps)) / 2.0)
      if (error1 <= 0.5) {
        errorOne += error1
      } else {
        ovNum1 += 1
      }
      var ref4 = ref(reverse(i * 4 + 3, log2Ceil(FFTLength)))
      var d4 = peek(c.io.dOut4)
      error1 = math.abs((((2 * d4("re").toDouble / bound) - ref4.re) / (ref4.re + eps) + ((2 * d4("im").toDouble / bound) - ref4.im) / (ref4.im + eps)) / 2.0)
      if (error1 <= 0.5) {
        errorOne += error1
      } else {
        ovNum1 += 1
